...
The E2E suite is designed to challenge the full Parallax pipeline with synthetic but varied market, fundamentals, news, corporate actions, portfolio, event, lifecycle, alerting, paper-trading ledger, team governance, partner-execution controls, beta-deployment API state, managed SaaS control-plane state, provider validation state, hosted API state, hosted console state, guided repair state, workspace onboarding state, account-console state, identity/session state, durable storage state, data-vendor state, LLM-provider state, local-alpha workspace, LLM council, and sandbox-execution data.
...
Evidence Snapshot
  ...
  -> Audit/Governance/Workspace/Alerts/Paper Lab/Team Governance/Partner Execution/Beta API/Managed SaaS/Provider Validation/Hosted API/Hosted Console/Guided Repair/Onboarding/Account Console/Identity/Storage/Data Vendor/LLM Provider/Sandbox paths where relevant
```
...
27. Phase 17 workspace onboarding can create hash-only invitations, accept an invite without the service token, issue a scoped identity session, let the accepted session read tenant routes, block duplicate acceptance, render console onboarding controls, and keep raw invite/session tokens out of persisted status.
28. Phase 18 account console can render public join links without embedding invite tokens, render a tenant console without control-plane calls, read/update account profiles, change membership roles, update active session scopes, deny reviewer analysis creation, and keep raw tokens out of public/status outputs.
